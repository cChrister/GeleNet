...
class GeleNet(nn.Module):
    ...
    def forward(self, x):
        ...
        score_list,score_PE = self.swin1(x)
        #score_PE = self.up3(score_PE)
        score_T_1 = score_list[0] #128*96*96
        #print(score_T_1.size())
        score_T_2 = score_list[1] #256*48*48
        #print(score_T_2.size())
        score_T_3 = score_list[2] #512*24*24
        #print(score_T_3.size())
        score_T_4 = score_list[3] #1024*12*12
        ...
        x_u1 = self.drop(x_u1) # 256*96*96

        x_u2 = self.resnet.layer2(x_u1)
        x_u2 = self.drop(x_u2) # 512*48*48

        x_u3 = self.resnet.layer3(x_u2)
        x_u3 = self.drop(x_u3) # 1024*24*24
