...
class VGGNetFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.vgg[10:17](x1)  # 216x44x44
...
class GeleNet(nn.Module):
    def __init__(self, channel=32):
        ...
        self.ChannelNormalization_v3 = BasicConv2d(256, channel, 3, 1, 1) # 320x22x22->32x22x22
        ...
        self.ChannelNormalization_t3 = BasicConv2d(128, channel, 3, 1, 1) # 320x22x22->32x22x22
        self.ChannelNormalization_t4 = BasicConv2d(320, channel, 3, 1, 1) # 512x11x11->32x11x11
    ...
    def forward(self, x):
        # x.size [1,3,352,352]
        # VGGNetFeatureExtractor
        v1, v2, v3, v4 = self.vgg(x)  # 128x88x88, 256x44x44, 512x22x22, 512x11x11
        # PVT backbone
        t1, t2, t3, t4= self.backbone(x) # 64x88x88, 128x44x44, 320x22x22, 512x11x11

        v1_nor = self.ChannelNormalization_v1(v1) # 32x88x88
        v3_nor = self.ChannelNormalization_v3(v2) # 32x22x22
        v4_nor = self.ChannelNormalization_v4(v3) # 32x11x11
        t1_nor = self.ChannelNormalization_t1(t1) # 32x88x88
        t3_nor = self.ChannelNormalization_t3(t2) # 32x22x22
        t4_nor = self.ChannelNormalization_t4(t3) # 32x11x11

        # # SWSAM for x4_nor
        # v4_SWSAM_4 = self.SWSAM_4(v4_nor)  # 32x11x11
        # # D-SWSAM for x1_nor
        # t1_ori = self.dirConv(t1_nor)
        # t1_DSWSAM_1 = self.DSWSAM_1(t1_ori) # 32x88x88

        # KTM for x2_nor and x3_nor
        vt1_KTM = self.KTM1(v1_nor, t1_nor) # 32x88x88
        # vt2_KTM = self.KTM_23(v2_nor, t2_nor)
        vt3_KTM = self.KTM2(v3_nor, t3_nor) # 32x22x22
        vt4_KTM = self.KTM3(v4_nor, t4_nor) # 32x11x11


        prediction = self.upsample_4(self.PDecoder(vt4_KTM, vt3_KTM, vt1_KTM))
